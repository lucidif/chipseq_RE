process MERGE_SPIKEIN_REF {
    ...
    output:
    path '*_merge.fa'       , emit: fasta
    path '*_merge.gtf'       , emit: gtf
    ...
    script:
        ...
        cat ${refname}_rename.fa ${spikename}_rename.fa > ${refname}_${spikename}_merge.fa

        cat ${gtf_ref} | sed -e 's/(chr)/${refname}_/g' > ${refname}_rename.gtf 
        cat ${gtf_spike} | sed -e 's/(chr)/${spikename}_/g' > ${spikename}_rename.gtf
        cat ${refname}_rename.gtf ${spikename}_rename.gtf > ${refname}_${spikename}_merge.gtf
